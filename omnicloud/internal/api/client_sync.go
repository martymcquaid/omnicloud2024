...
func NewClientSync(database *db.DB, serverID uuid.UUID, mainServerURL, serverName, serverLocation, registrationKey, softwareVersion string) (*ClientSync, error) {
...
func (cs *ClientSync) registerWithMainServer() {
	...
	if err := json.NewDecoder(resp.Body).Decode(&regResponse); err == nil {
		...
		if !regResponse.IsAuthorized {
			...
			log.Printf("✓ Successfully registered and AUTHORIZED with main server (Packages: %d, MAC: %s)", packageCount, cs.macAddress)
		}
	} else {
